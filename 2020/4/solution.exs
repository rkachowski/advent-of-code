defmodule Day4 do
  ...
  def valid("hcl", haircolor) when byte_size(haircolor) == 7,
    do: Regex.run(~r/#[0-9a-f]{6}/, haircolor) != nil

  def valid("ecl", eyecolor) when eyecolor in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"],
    do: true

  def valid("pid", passportid) when byte_size(passportid) == 9,
    do: Regex.run(~r/\d{9}/, passportid) != nil
