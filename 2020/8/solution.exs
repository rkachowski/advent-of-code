defmodule Day8 do
  ...
  def brute_force [val | tail], input do
    modifed = List.update_at(input, val, fn
  ...
  def solve1(input, {index, acc, seen}) when index >= length(input), do:
    {:end, {index, acc, seen}}
  ...
  def run [ _, "acc", val], {index, acc, seen} do
    {index + 1, acc + String.to_integer(val), MapSet.put(seen, index)}
  end

  def run [ _, "jmp", val ], {index, acc, seen} do
    {index + String.to_integer(val), acc, MapSet.put(seen, index)}
  end

  def run [ _, "nop", _ ], {index, acc, seen} do
    {index + 1, acc, MapSet.put(seen, index)}
  end
end
